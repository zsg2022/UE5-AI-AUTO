...
static TSharedPtr<FJsonObject> MakeObj(){return MakeShareable(new FJsonObject());}
static TSharedPtr<FJsonValue> MakeObjVal(TSharedPtr<FJsonObject> J){return MakeShareable(new FJsonValueObject(J));}
#define RETURN_ERR(M) do{auto E=MakeObj();E->SetStringField(TEXT("error"),M);return E;}while(0)
...
UBlueprint* FUE5AIAUTOBlueprintEditor::GetCachedBP(const FString& Path){
	...
	if(!BP)BP=FindObject<UBlueprint>(nullptr,*Path);if(!BP){FString FullPath=FPaths::GetBaseFilename(Path);BP=FindObject<UBlueprint>(nullptr,*(Path+TEXT(".")+FullPath));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListNodeClasses(){
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListBlueprints(const FString& Path){
	...
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> Arr;
	for(const auto& D:A){auto J=MakeObj();J->SetStringField(TEXT("name"),D.AssetName.ToString());J->SetStringField(TEXT("path"),D.GetObjectPathString());Arr.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::OpenBlueprint(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	auto R=MakeObj();R->SetStringField(TEXT("name"),B->GetName());
	R->SetStringField(TEXT("parent"),B->ParentClass?B->ParentClass->GetName():TEXT("None"));
	TArray<TSharedPtr<FJsonValue>> GA;
	for(UEdGraph* G:B->UbergraphPages){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetStringField(TEXT("type"),TEXT("EventGraph"));GA.Add(MakeObjVal(J));}
	for(UEdGraph* G:B->FunctionGraphs){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetStringField(TEXT("type"),TEXT("Function"));GA.Add(MakeObjVal(J));}
	for(UEdGraph* G:B->MacroGraphs){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetStringField(TEXT("type"),TEXT("Macro"));GA.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::CreateBlueprint(const FString& Path,const FString& PC){
	UClass* P=UClass::TryFindTypeSlow<UClass>(PC);
	if(!P)P=UClass::TryFindTypeSlow<UClass>(FString(TEXT("U"))+PC);
	if(!P)P=AActor::StaticClass();
	...
	if(!BP)RETURN_ERR("Create failed");
	LoadedBPs.Add(F/N,BP);Pkg->SetDirtyFlag(true);FAssetRegistryModule::AssetCreated(BP);
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("created"));R->SetStringField(TEXT("path"),F/N);return R;
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::CompileBlueprint(const FString& P){
	...
	auto R=MakeObj();R->SetStringField(TEXT("status"),B->Status==BS_UpToDate?TEXT("ok"):TEXT("has_errors"));return R;
}

TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::SaveBlueprint(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	B->GetOutermost()->SetDirtyFlag(true);
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("dirty"));return R;
}

TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListNodes(const FString& P,const FString& GName){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("BP not found");
	UEdGraph* G=_Graph(B,GName);if(!G)RETURN_ERR("Graph not found");
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
	for(UEdGraphNode* N:G->Nodes){
		auto J=MakeObj();J->SetStringField(TEXT("id"),N->NodeGuid.ToString());J->SetStringField(TEXT("class"),N->GetClass()->GetName());
		...
		for(UEdGraphPin* Pin:N->Pins){auto PJ=MakeObj();PJ->SetStringField(TEXT("name"),Pin->PinName.ToString());PJ->SetStringField(TEXT("type"),Pin->PinType.PinCategory.ToString());PJ->SetStringField(TEXT("dir"),Pin->Direction==EGPD_Input?TEXT("In"):TEXT("Out"));if(Pin->DefaultValue.Len())PJ->SetStringField(TEXT("default"),Pin->DefaultValue);PJ->SetNumberField(TEXT("connected"),Pin->LinkedTo.Num());PA.Add(MakeObjVal(PJ));}
		J->SetArrayField(TEXT("pins"),PA);A.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::CreateNode(const FString& P,const FString& GName,const FString& NC,double X,double Y,TSharedPtr<FJsonObject> Defs){
	...
	if(!NClass)NClass=FindObject<UClass>(nullptr,*(TEXT("/Script/BlueprintGraph.K2Node_")+NC));
	if(!NClass)for(TObjectIterator<UClass> It;It;++It)if(It->IsChildOf(UK2Node::StaticClass())&&(It->GetName()==NC||It->GetName()==FString(TEXT("K2Node_"))+NC)){NClass=*It;break;}
	if(!NClass||!NClass->IsChildOf(UK2Node::StaticClass())){if(UFunction* UF=_Function(NC)){NClass=UK2Node_CallFunction::StaticClass();if(!Defs.IsValid())Defs=MakeShareable(new FJsonObject());Defs->SetStringField(TEXT("function"),NC);}else RETURN_ERR(FString::Printf(TEXT("Node class not found: %s"),*NC));}
	...
	if(Defs.IsValid()){
		if(Defs->HasField(TEXT("function"))){if(auto* CF=Cast<UK2Node_CallFunction>(Node)){if(auto* UF=_Function(Defs->GetStringField(TEXT("function"))))CF->SetFromFunction(UF);else{G->RemoveNode(Node);RETURN_ERR(FString::Printf(TEXT("Function not found: %s"),*Defs->GetStringField(TEXT("function"))));}}}
	...
	auto R=MakeObj();R->SetStringField(TEXT("id"),Node->NodeGuid.ToString());R->SetStringField(TEXT("class"),Node->GetClass()->GetName());return R;
}

TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::RemoveNode(const FString& P,const FString& GName,const FString& Id){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("BP not found");UEdGraph* G=_Graph(B,GName);if(!G)RETURN_ERR("Graph not found");
	UK2Node* N=_Node(G,Id);if(!N)RETURN_ERR("Node not found");G->RemoveNode(N);FBlueprintEditorUtils::MarkBlueprintAsModified(B);
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("removed"));return R;
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::SetPinDefault(const FString& P,const FString& GName,const FString& NId,const FString& PinName,const FString& Val){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("BP not found");UEdGraph* G=_Graph(B,GName);if(!G)RETURN_ERR("Graph not found");
	UK2Node* N=_Node(G,NId);if(!N)RETURN_ERR("Node not found");UEdGraphPin* Pin=_Pin(N,PinName);if(!Pin)RETURN_ERR("Pin not found");
	Pin->DefaultValue=Val;FBlueprintEditorUtils::MarkBlueprintAsModified(B);
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("set"));return R;
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ConnectPins(const FString& P,const FString& GName,const FString& SN,const FString& SP,const FString& DN,const FString& DP){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("BP not found");UEdGraph* G=_Graph(B,GName);if(!G)RETURN_ERR("Graph not found");
	UK2Node* S=_Node(G,SN),*D=_Node(G,DN);if(!S||!D)RETURN_ERR("Node not found");
	UEdGraphPin* Ps=_Pin(S,SP),*Pd=_Pin(D,DP);if(!Ps||!Pd)RETURN_ERR("Pin not found");
	if(Ps->Direction==Pd->Direction)RETURN_ERR("Same direction");
	G->GetSchema()->TryCreateConnection(Ps,Pd);FBlueprintEditorUtils::MarkBlueprintAsModified(B);
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("connected"));return R;
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::DisconnectPin(const FString& P,const FString& GName,const FString& NId,const FString& PinName){
	...
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("disconnected"));return R;
}

TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListVariables(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
	for(const auto& V:B->NewVariables){auto J=MakeObj();J->SetStringField(TEXT("name"),V.VarName.ToString());J->SetStringField(TEXT("type"),V.VarType.PinCategory.ToString());J->SetStringField(TEXT("default"),V.DefaultValue);A.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::CreateVariable(const FString& P,const FString& N,const FString& T,bool bA){
	...
	if(!bValid)RETURN_ERR(FString::Printf(TEXT("Unknown type: %s"),*T));
	...
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("created"));R->SetStringField(TEXT("name"),N);return R;
}
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::RemoveVariable(const FString& P,const FString& N){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	for(int32 i=0;i<B->NewVariables.Num();++i)if(B->NewVariables[i].VarName==*N){B->NewVariables.RemoveAt(i);FBlueprintEditorUtils::MarkBlueprintAsModified(B);auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("removed"));return R;}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::SetVariableDefault(const FString& P,const FString& N,const FString& V){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	for(FBPVariableDescription& VD:B->NewVariables)if(VD.VarName==*N){VD.DefaultValue=V;FBlueprintEditorUtils::MarkBlueprintAsModified(B);auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("set"));return R;}
	RETURN_ERR("Not found");
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListFunctions(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
	for(UEdGraph* G:B->FunctionGraphs){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetNumberField(TEXT("nodes"),G->Nodes.Num());A.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::RemoveFunction(const FString& P,const FString& N){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	for(UEdGraph* G:B->FunctionGraphs)if(G->GetName()==N){FBlueprintEditorUtils::RemoveGraph(B,G);FBlueprintEditorUtils::MarkBlueprintAsModified(B);auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("removed"));return R;}
	RETURN_ERR("Not found");
}
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListGraphs(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
	for(UEdGraph* G:B->UbergraphPages){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetStringField(TEXT("type"),TEXT("EventGraph"));J->SetNumberField(TEXT("nodes"),G->Nodes.Num());A.Add(MakeObjVal(J));}
	for(UEdGraph* G:B->FunctionGraphs){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetStringField(TEXT("type"),TEXT("Function"));J->SetNumberField(TEXT("nodes"),G->Nodes.Num());A.Add(MakeObjVal(J));}
	for(UEdGraph* G:B->MacroGraphs){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetStringField(TEXT("type"),TEXT("Macro"));J->SetNumberField(TEXT("nodes"),G->Nodes.Num());A.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListComponents(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
	if(B->SimpleConstructionScript)for(auto* N:B->SimpleConstructionScript->GetAllNodes()){auto J=MakeObj();J->SetStringField(TEXT("name"),N->GetVariableName().ToString());J->SetStringField(TEXT("class"),N->ComponentClass?N->ComponentClass->GetName():TEXT("None"));A.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::AddComponent(const FString& P,const FString& CC,const FString& VN){
	...
	if(!Cls||!Cls->IsChildOf(UActorComponent::StaticClass()))RETURN_ERR(FString::Printf(TEXT("Component not found: %s"),*CC));
	...
	auto* Node=B->SimpleConstructionScript->CreateNode(Cls,FName(*VN));if(!Node)RETURN_ERR("CreateNode failed");
	B->SimpleConstructionScript->AddNode(Node);FBlueprintEditorUtils::MarkBlueprintAsModified(B);
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("added"));R->SetStringField(TEXT("name"),VN);return R;
}
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::RemoveComponent(const FString& P,const FString& N){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");if(!B->SimpleConstructionScript)RETURN_ERR("No SCS");
	for(USCS_Node* Nd:B->SimpleConstructionScript->GetAllNodes())if(Nd->GetVariableName()==*N){B->SimpleConstructionScript->RemoveNode(Nd);FBlueprintEditorUtils::MarkBlueprintAsModified(B);auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("removed"));return R;}
	RETURN_ERR("Not found");
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListInterfaces(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
	for(const auto& I:B->ImplementedInterfaces){auto J=MakeObj();J->SetStringField(TEXT("class"),I.Interface?I.Interface->GetName():TEXT("None"));A.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::AddInterface(const FString& P,const FString& IC){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	UClass* Cls=FindObject<UClass>(nullptr,*IC);if(!Cls||!Cls->IsChildOf<UInterface>())RETURN_ERR(FString::Printf(TEXT("Interface not found: %s"),*IC));
	FBlueprintEditorUtils::ImplementNewInterface(B,FName(*IC));FBlueprintEditorUtils::MarkBlueprintAsModified(B);
	auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("implemented"));R->SetStringField(TEXT("interface"),IC);return R;
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::ListMacros(const FString& P){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	auto R=MakeObj();TArray<TSharedPtr<FJsonValue>> A;
	for(UEdGraph* G:B->MacroGraphs){auto J=MakeObj();J->SetStringField(TEXT("name"),G->GetName());J->SetNumberField(TEXT("nodes"),G->Nodes.Num());A.Add(MakeObjVal(J));}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::RemoveMacro(const FString& P,const FString& N){
	UBlueprint* B=GetCachedBP(P);if(!B)RETURN_ERR("Not found");
	for(UEdGraph* G:B->MacroGraphs)if(G->GetName()==N){FBlueprintEditorUtils::RemoveGraph(B,G);FBlueprintEditorUtils::MarkBlueprintAsModified(B);auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("removed"));return R;}
	RETURN_ERR("Not found");
}
...
TSharedPtr<FJsonObject> FUE5AIAUTOBlueprintEditor::RemoveEventDispatcher(const FString& P,const FString& N){
	...
	{B->NewVariables.RemoveAt(i);FBlueprintEditorUtils::MarkBlueprintAsModified(B);auto R=MakeObj();R->SetStringField(TEXT("status"),TEXT("removed"));return R;}
	RETURN_ERR("Not found");
}
